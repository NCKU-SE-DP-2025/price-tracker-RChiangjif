...
class NewsScrapingService:
    def get_news_info(search_term, is_initial=False):
        ...
        if is_initial:
            ...
            for page_number in range(1, 10):
                ...
                page_result.append(response.json()["lists"])

            for l in page_result:
    ...
    def get_news(is_initial=False):
        ...
        :return:
        """
        news_data = NewsScrapingService.get_news_info("價格", is_initial=is_initial)
        for news in news_data:
            ...
            if relevance == "high":
                ...
                DatabaseService.add_news(detailed_news)
...
class UserService:
    ...
    def verify_user_password(db, username, pwd):
        user = db.query(User).filter(User.username == username).first()
        if user == None:
            return False
        if not UserService.verify_password(pwd, user.hashed_password):
            return False
        return user
...
def start_scheduler():
    ...
    if db.query(NewsArticle).count() == 0:
        ...
        NewsScrapingService.get_news()
    db.close()
    background_scheduler.add_job(NewsScrapingService.get_news, "interval", minutes=100)
...
def read_users_me(user=Depends(UserService.authenticate_user_token)):
...
def read_news(db=Depends(get_db)):
    ...
    result = []
    for article in news: 
        upvotes, upvoted = DatabaseService.get_article_upvote_details(article.id, None, db)
        result.append(
...
def read_user_news(
        db=Depends(get_db),
        u=Depends(UserService.authenticate_user_token)
):
    """
    read user new
    ...
    result = []
    for article in news:
        upvotes, upvoted = DatabaseService.get_article_upvote_details(article.id, u.id, db)
        result.append(
            {
                **article.__dict__,
                "upvotes": upvotes,
...
async def search_news(request: PromptRequest):
    ...
    # should change into simple factory pattern
    news_items = NewsScrapingService.get_news_info(keywords, is_initial=False)
...
async def news_summary(
        payload: NewsSumaryRequestSchema, u=Depends(UserService.authenticate_user_token)
):
    response = {}
    summary_messages = [
        {
...
def upvote_article(
        id,
        db=Depends(get_db),
        user=Depends(UserService.authenticate_user_token),
):
